"""Run the basic AO system example pipeline."""
...
def main() -> None:
    ...
    logger = logging.getLogger("shmpipeline.example.ao")
    ...
    logger.info("building AO example pipeline from %s", config_path)
    manager.build()
    manager.start()

    try:
        manager.get_stream("ao_centroid_offset").write(centroid_offset)
        manager.get_stream("ao_reconstructor_matrix").write(reconstructor_matrix)
        manager.get_stream("ao_affine_offset").write(affine_offset)
        logger.info("loaded static AO calibration streams")
        ...
        for index in range(frame_count):
            ...
            manager.get_stream("ao_sensor_image").write(image)
            dm_command = wait_for_next_write(
                manager.get_stream("ao_dm_command"),
                baseline,
                timeout=2.0,
            )

            observed_centroids = manager.get_stream("ao_centroids").read()
            observed_corrected = manager.get_stream("ao_corrected_centroids").read()
            observed_flattened = manager.get_stream("ao_flattened_slopes").read()
            observed_open_loop = manager.get_stream("ao_open_loop_command").read()
            ...
            if index == 0:
                ...
                logger.info("verified %d/%d AO frames", index + 1, frame_count)

        elapsed = time.perf_counter() - start
        logger.info(
            "AO verification complete: frames=%d elapsed=%.3fs throughput=%.1f frames/s",
            frame_count,
            elapsed,
            frame_count / elapsed,
        )
        print(f"Verified {frame_count} AO frames in {elapsed:.3f}s")
    finally:
