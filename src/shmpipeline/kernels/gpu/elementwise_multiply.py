"""GPU elementwise multiplication kernel."""
...
class ElementwiseMultiplyGpuKernel(GpuKernel):
    """Compute output = input * auxiliary elementwise."""

    kind = "gpu.elementwise_multiply"
    auxiliary_arity = 1
    ...
    def compute_into(
        ...
        torch.mul(as_gpu_tensor(trigger_input, device=self.device), rhs, out=output)
